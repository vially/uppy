...
describe('core/utils', function () {
  describe('generateFileID', function () {
    it('should take the filename object and produce a lowercase file id made up of uppy- prefix, file name (cleaned up to be lowercase, letters and numbers only), type, size and lastModified date', function () {
      ...
      expect(utils.generateFileID(fileObj)).toEqual('uppy-foo0fijpg-image/jpeg-2271173-1498510508000');
    });
  });
  ...
  describe('toArray', function () {
    it('should convert a array-like object into an array', function () {
      ...
      expect(utils.toArray(obj)).toEqual(['zero', 'one', 'two', 'three', 'four']);
    });
  });
  ...
  describe('runPromiseSequence', function () {
    it('should run an array of promise-returning functions in sequence', function () {
      ...
      return utils.runPromiseSequence([promiseFn1, promiseFn2, promiseFn3]).then(function () {
  ...
  describe('getFileTypeExtension', function () {
    it('should return the filetype based on the specified mime type', function () {
  ...
  describe('getArrayBuffer', function () {
    ...
    it('should return a promise that resolves with the specified chunk', function () {
      return utils.getArrayBuffer('abcde').then(function (buffer) {
  ...
  describe('dataURItoBlob', function () {
    it('should convert a data uri to a blob', function () {
      var blob = utils.dataURItoBlob(sampleImageDataURI, {});
  ...
  describe('dataURItoFile', function () {
    it('should convert a data uri to a file', function () {
      var file = utils.dataURItoFile(sampleImageDataURI, { name: 'foo.jpg' });
  ...
  describe('prettyETA', function () {
    it('should convert the specified number of seconds to a pretty ETA', function () {
  ...
  describe('getSocketHost', function () {
    it('should get the host from the specified url', function () {
      expect(utils.getSocketHost('https://foo.bar/a/b/cd?e=fghi&l=k&m=n')).toEqual('ws://foo.bar/a/b/cd?e=fghi&l=k&m=n');
    });
  });

  describe('settle', function () {
    it('should resolve even if all input promises reject', function () {
      return expect(utils.settle([_Promise.reject(new Error('oops')), _Promise.reject(new Error('this went wrong'))])).resolves.toMatchObject({
        successful: [],
    ...
    it('should resolve with an object if some input promises resolve', function () {
      return expect(utils.settle([_Promise.reject(new Error('rejected')), _Promise.resolve('resolved'), _Promise.resolve('also-resolved')])).resolves.toMatchObject({
  ...
  describe('limitPromises', function () {
    ...
    it('should run at most N promises at the same time', function () {
      var limit = utils.limitPromises(4);
      var fn2 = limit(fn);
    ...
    it('should accept Infinity as limit', function () {
      var limit = utils.limitPromises(Infinity);
      var fn2 = limit(fn);
